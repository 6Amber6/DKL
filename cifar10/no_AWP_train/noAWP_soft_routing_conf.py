...
parser.add_argument('--mark', type=str, default='soft_routing_conf_noawp')
...
def train_dkl_epoch(model, loader, optimizer, epoch, ema, weight, lut4, lut6, device):
    model.train()
    losses = AverageMeter()
    top1 = AverageMeter()
...
def main():
    ...
    for ep in range(start_epoch_dkl, args.epochs_fusion + 1):
        ...
        loss_avg, acc_avg, weight = train_dkl_epoch(
            fusion, train_loader_10, optimizer, ep, ema, weight, lut4, lut6, device
        )
