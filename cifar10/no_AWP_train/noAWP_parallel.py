"""
DKL-based Parallel WRN Training for CIFAR-10 (no AWP, EMA).
...
parser = argparse.ArgumentParser(description='DKL Parallel WRN CIFAR-10 (no AWP, EMA)')
...
parser.add_argument('--mark', type=str, default='parallel_noawp')
parser.add_argument('--data-path', type=str, default='../data')
parser.add_argument('--awp-warmup', type=int, default=999, help='disabled (999)')
...
parser.add_argument('--no-ema', action='store_false', dest='use_ema', help='disable EMA')
...
def train_dkl_epoch(model, loader, optimizer, epoch, ema, weight, device):
    model.train()
    losses = AverageMeter()
    top1 = AverageMeter()
...
def main():
    ...
    for ep in range(start_epoch_dkl, args.epochs_fusion + 1):
        ...
        loss_avg, acc_avg, weight = train_dkl_epoch(
            fusion, train_loader_10, optimizer, ep, ema, weight, device
        )
        scheduler.step()
