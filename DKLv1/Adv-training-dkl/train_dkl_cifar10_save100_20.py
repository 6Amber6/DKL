...
parser.add_argument('--save-freq', '-s', default=20, type=int, metavar='N',
                    help='save every N epochs (from save-start)')
parser.add_argument('--save-start', default=100, type=int, metavar='N',
                    help='first epoch to save (then every save-freq), e.g. 100+20 -> 100,120,...,200')
# AWP
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        # save checkpoint: from save_start every save_freq (e.g. 100,120,140,160,180,200 -> 6 saves)
        if epoch >= args.save_start and (epoch - args.save_start) % args.save_freq == 0:
